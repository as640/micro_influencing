...
const quickLinks = [
  ...
  { to: '/signup', label: 'Sign Up Creators' },
  ...
  return (
    <footer className="mt-12 border-t border-slate-800 bg-slate-900/90 px-6 py-10 text-slate-300 animate-fade-up">
      <div className="grid gap-10 md:grid-cols-4">
        <div>
          <p className="text-lg font-bold text-white">MicroFluence</p>
          <p className="mt-2 text-xs text-slate-400">Grow together with smarter creator-brand collaborations.</p>
          <div className="mt-4 flex items-center gap-2">
            {socialLinks.map((item) => (
              <a
                ...
                className="rounded-lg border border-slate-700 bg-slate-800 p-2 text-slate-300 transition hover:border-indigo-500 hover:text-white"
              >
                {item.icon}
              </a>
            ))}
        ...
        <div>
          <p className="text-sm font-bold text-white uppercase tracking-wider">Resources</p>
          <div className="mt-4 space-y-2.5 text-sm">
            <Link to="/insights" className="block transition hover:text-indigo-300">Insights</Link>
            <Link to="/careers" className="block transition hover:text-indigo-300">Careers</Link>
            <Link to="/our-services" className="block transition hover:text-indigo-300">Our Services</Link>
            <Link to="/faq" className="block transition hover:text-indigo-300">FAQ</Link>
          </div>
        </div>

        <div>
          <p className="text-sm font-bold text-white uppercase tracking-wider">Get In Touch</p>
          <div className="mt-4 space-y-2.5 text-sm">
            <a href="mailto:support@microfluence.com" className="block transition hover:text-indigo-300">Contact Us</a>
            {quickLinks.map((item) => (
              <Link key={item.to} to={item.to} className="block transition hover:text-indigo-300">
                {item.label}
              </Link>
